...
# ADR-009 decision 4: the confirmation window per action. Twice the relayed
# latency on file (7 s stop->finishing, 10 s start->charging) and the values
# the contributor's own integration tested live against real hardware, so
# the first hardware run from this integration compares like for like.
