...
>
> **The Ocean 2 is read-only, and its field map rests on one installation.** Every reading was checked frame for frame against a seven-hour recording from one owner's `RE11`, with the mapping worked out by the author of a standalone Ocean 2 integration. An `RE17` is the same product at a higher rating and gets the same entities, but no `RE17` recording is on file yet, so the first report from one is the check. The battery modules are read from their own report and keyed on their index; two modules are on file and six are defined. Not one write to this device has been observed on the wire, so there are no controls. See [entity reference](documentation/entities/ocean-2.md).
>
...
- **PowerPulse 2 wallbox** (`C376`) - its own device, no PowerOcean required: charging power, voltage and current per phase, the configured current limit, the phase mode, the start, duration, energy and start meter reading of the running session, the lifetime energy counter, the charging state and whether the cable lock is on. Enhanced Mode only: the wallbox reports on its own channel of the account connection, and the Developer API refuses it with error 1006. See [PowerPulse 2](documentation/entities/powerpulse-2.md).
