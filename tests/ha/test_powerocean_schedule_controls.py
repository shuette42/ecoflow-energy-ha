...
class TestTheNumber:
    ...
    async def test_the_range_is_the_one_the_builder_validates_against(
        self, hass: HomeAssistant
    ) -> None:
        """The bound is ours, not the device's, and the two have to agree or a
        slider position would be refused only once it reached the builder."""
        ...
        assert number.native_min_value == TIMER_TASK_POWER_MIN_W
        assert number.native_max_value == TIMER_TASK_POWER_MAX_W
