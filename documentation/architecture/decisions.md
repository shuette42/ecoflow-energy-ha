...
### Addendum 2026-09-12: the own-topic write is evidenced

**Both halves of the sentence above are reversed by the ADR-009 addendum of the same day.** A recording of the app starting and stopping a wallbox on an account without a PowerOcean shows the command on the wallbox's own set topic, acknowledged by the wallbox and followed by the state change on its heartbeat. A wallbox without a PowerOcean now gets the same two buttons on that route. What this paragraph still keeps: the settings writes stay out of scope.
...
### Addendum 2026-09-12: the wallbox accepts the command on its own topic, and a wallbox without a PowerOcean gets the same two buttons

**What arrived.** Decision 8 named the one thing that could open the path for a wallbox on an account without a PowerOcean: a recording of the app starting and stopping such a wallbox. The owner of that setup delivered one on 2026-09-12: 67 minutes on v1.21.0, 1460 frames seen, 140 kept, with the app's writes on the wallbox's own topic in the buffer. It holds the app's command five times, in one message of the wallbox's own family on the wallbox's own set topic, addressed to module 2 with the wallbox's own serial in the envelope and one selector field: `2` for stop, `1` for start, the mirror image of the relayed command's values. The wallbox answers every one with an empty acknowledgement carrying the same sequence number, which the relayed route never had. Laid against the wallbox's own heartbeat, the five writes read stop, start, stop, start, stop: the state changes to `finishing` 1 to 2 s after each stop and to `charging` 17 s after the start whose following frames survived the buffer. Both latencies sit inside the windows of decision 4. The five frames are reproduced byte for byte by the builder and pinned in the tests.

**The context paragraph that no longer holds.** "Whether the wallbox accepts writes on its own topic is unevidenced" was true on 2026-09-11 and is not now. The same recording also settles a smaller point: the four frames of another message that appear each time the app opens the wallbox page carry nothing but a timestamp and are the app's time sync, not a command.

**What changes, by decision:**

- **Decision 1 stands, and its criterion now picks the route.** The command goes where the device accepts it. A wallbox behind a PowerOcean accepts it through the PowerOcean, addressed by the descriptor; a wallbox on its own accepts it on its own topic. Both confirmations come from the wallbox's own heartbeat, both on one transport, one account, one mode.
- **Decision 2 keeps its invariant.** A connection publishes only to its own serial. On the route through a PowerOcean the wallbox coordinator still publishes through its sibling; on its own it publishes through its own connection to its own serial, the path every account-sign-in coordinator already has. No connection gained a way to publish to another serial.
- **Decision 3 becomes: one PowerOcean or none; two or more, no control.** The entry's PowerOcean count now selects one of three outcomes. Exactly one: the relayed command through it. None: the command on the wallbox's own topic. Two or more: no buttons, unchanged, since neither route has been observed on such an account. An entry whose coordinator table is already gone, which is what a teardown looks like from inside a press, selects none of the three and the press fails as not delivered with zero publishes on either route; a review found that reading "no PowerOcean" from an absent table would have sent the own-topic command during every unload.
- **Decision 6 keeps its shape, with the gate and the availability following the route.** With a PowerOcean, the buttons appear on the first descriptor report and their availability follows the sibling's connection, as before. Without one, there is no descriptor to wait for: such an account never sends the settings message that carries it, so the buttons appear on the first heartbeat whose plug status the parser maps, and their availability follows the wallbox's own connection. The state precondition stays outside availability on both routes.
- **Decision 7 applies to the relayed route only.** The own-topic command carries no descriptor: it needs the wallbox's own serial, which the coordinator has from the account's device list.
- **Decision 8 is reversed.** A wallbox without a PowerOcean gets the same two buttons, with the same preconditions and the same windows.
...
### Addendum 2026-09-13: the first settings write, the maximum current, follows the same shape through a PowerOcean

**What arrived.** Nothing new from the device; two recordings already on file, laid side by side for the first time. The settings recording of 2026-08-24 (#7) holds seven app writes of the wallbox's settings message on the PowerOcean's set topic, addressed to the accessory exactly like the start and stop command, with one nested field for the maximum current in tenths of an ampere. About a second after each write the wallbox answers on its own topic with its settings report, and that report carries the written value in a field of its own; the next heartbeat carries it again in the field the Wallbox Maximum Current sensor already reads. The owner's sweep of 2026-09-10 across 6 to 16 A (#7) shows the same echo four more times. Nothing on the wallbox's own topic moves with the operating mode, and no recording holds a write of this message on the wallbox's own channel.

**What changes.** The sentence of the addendum above, "the settings writes (current, mode, phase, target) stay out of scope", is narrowed: the maximum current is decided by this addendum and the other three stay out of scope. Decision 1's criterion applies unchanged: the command goes where the device accepts it, through the PowerOcean, and the confirmation comes from the value the entity already shows, the wallbox's own settings report or heartbeat. Decision 3 applies as it stands after the 2026-09-12 addendum, but only its first outcome: exactly one PowerOcean in the entry, the relayed write. None: no control, because the recording of the wallbox without a PowerOcean (2026-09-12) holds no settings write on its own channel, and an unevidenced route gets nothing, the same reason decision 8 held until a recording arrived. Two or more: no control, as for the buttons. Decision 4's confirmation runs on the arriving frame, never on the store, extended from a state in a set to a value equal to the one written; a stale reading of the same value cannot confirm a write. Decision 6's availability follows the PowerOcean that carries the write. Decision 7's descriptor is required, as on the relayed route for the buttons.

**Window.** Twenty seconds, ten times the one to two seconds measured on the settings report over eleven writes in two recordings, and below the heartbeat interval so that a wallbox which stops answering on its settings report fails with a message rather than succeeding late on a heartbeat.

**Consequences:** a third builder on the same envelope, this one carrying the PowerOcean's serial in the envelope like every other write on that topic; one number entity sharing the sensor's key, so the control shows what the sensor shows and applies nothing on its own; the pending record names the key and the value it waits for, so a start and a current write cannot confirm each other. The operating mode is not decided here: its only echo is on the PowerOcean's topic, and its labels wait for a recording the owner has been asked for.

### Addendum 2026-09-14: the charging mode follows, read back on the wallbox's own heartbeat, and one of its four values stays with the app

**What arrived.** The recording asked for in the addendum above, the same evening (#7, 2026-09-13). The owner switched his wallbox through all four charging modes from the app with a minute between them, and the download holds every one of the five writes with its reply: the same settings message as the current, through the PowerOcean, with the mode in a field of its own. Fast is 1, Solar 2, Custom 3, Smart 4, the app's own labels against the app's own writes. The sentence above that nothing on the wallbox's own topic moves with the mode was wrong, and the recording shows where to look: the wallbox's heartbeat carries a linkage block that names the PowerOcean it is coupled to, and that block carries the mode. It read 2 ten seconds after the switch to Solar, 4 less than a second after the switch to Smart, 2 again a second and a half after the switch back, and 3 fifty-six seconds after the switch to Custom, on the next heartbeat the recorder kept. The heartbeat's own timing is the only delay: the wallbox sends one about once a minute and sends one at once after some writes.

The recording also shows what the app puts beside the mode. For Fast it writes the mode alone. For Custom it repeats the configured current, for Solar the flag bits and the solar minimum, for Smart a departure time, a target type and a target, and the heartbeat then carries those Smart values in a block of their own.

**What changes.** The addendum above narrowed the out-of-scope sentence to three settings; this one narrows it to two, phase and target. The mode is decided by the same criteria: the command goes through the PowerOcean because that is where the app puts it, the confirmation comes from the wallbox's own heartbeat because that is where the wallbox reports it, and only the entry with exactly one PowerOcean gets the control. Two things are particular to the mode.

The integration writes the mode field alone, for every mode it writes, which is the shape the app uses for Fast and a narrower shape than the app uses for Solar and Custom. The values the app repeats beside those two are the wallbox's current settings, echoed back, and a settings field left out of a write leaves the setting as it is. That is a reading of the message and not an observation, so the confirmation carries the risk: a wallbox that ignores the bare write reports no new mode, and the control fails with a message naming the mode it still reports. The owner's first switch from Home Assistant on the pre-release is the observation; if it fails, the write grows the two echoed fields from the heartbeat's own values, which is a change to the builder and to nothing else.

Smart is shown and not written. The app never sends the Smart mode without a departure time and a target, and a write without them has not been observed; a select that offered Smart would either send something the app never sends or invent a target. The option stays in the list so the control can show the mode the wallbox is in, and choosing it fails at once with a message pointing to the app. A Wallbox Charging Mode sensor shows the same value on every entry, including the one without a PowerOcean, which gets no select.
...
3. **The own connection wins for 60 s after it last delivered the block.** The two deliveries of one unit's entry differ: the owner's connection carries the strings fractional with the total equal to their sum, the neighbour's carries them in whole watts. Last-writer-wins would alternate a unit's reading between the two copies at the push cadence, by the 1 to 2 W the relayed total sits off the sum on every relayed entry on file that carries all four strings, and double the recorder writes for no information. A handed entry is dropped, and counted as held, while the receiver's own connection delivered that block less than 60 s ago; an own entry always applies. The anchor is the time the block last arrived on the own connection, per block, not the connection's last message: the failing unit pushed a frame every two seconds all night without the block, and a hold anchored on liveness would have held the relayed block out for good.
4. **An absent string is read as idle only when the entry's total says so.** On the unit's own copy an omitted string is an idle one: on every daylight entry on file that misses a string (8 of 8 across two single-unit captures) the strings present sum to the total exactly, and the app showed 0 W for the missing one, so the fill to 0 W stays. On the relayed copy the total is the discriminator: the fill runs while the strings present account for the total within 2.5 W, the rounding the relayed copy carries (1 to 2 W on all 14 relayed entries with four strings); when they do not (07:36:38: 217 W present against 239 stated), the missing string was producing and its key stays out, so the sensor keeps its last reading. The night shape, no total and no string, still fills all five to 0 W.
...
- (-) A string omitted from a relayed entry whose total does not account for it keeps its last value rather than showing what it is; the device's own total says it is producing, and nothing on the wire says how much. The 2.5 W slack is measured on 14 relayed entries, not chosen
...
4. Derive the missing string from the total. Rejected: the relayed copy's total is itself rounded and sits 1 to 2 W off the sum on frames where all four strings are present, so the derived value would be a guess presented as a reading.
