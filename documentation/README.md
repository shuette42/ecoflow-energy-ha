...
- [PowerPulse 2](entities/powerpulse-2.md) - 18 sensors, 1 binary sensor, 1 number, 1 select, 2 buttons (`C376`, `C374`) - a wallbox; start and stop with or without a PowerOcean, the maximum current and the charging mode with exactly one PowerOcean in the same integration entry, Enhanced Mode only, no PowerOcean required for the readings
