...
- [PowerPulse 2](entities/powerpulse-2.md) - 17 sensors, 1 binary sensor, 1 number, 2 buttons (`C376`, `C374`) - a wallbox; start and stop with or without a PowerOcean, the maximum current with exactly one PowerOcean in the same integration entry, Enhanced Mode only, no PowerOcean required for the readings
