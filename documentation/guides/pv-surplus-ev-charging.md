...
This guide turns four PowerOcean readings this integration already provides into a wallbox automation: the car charges when the roof produces more than the house and the home battery take, and stops when a cloud or the evening ends that. It is written for a wallbox that Home Assistant controls, which is any wallbox with its own integration, and it says up front what is different when the wallbox is an EcoFlow PowerPulse.

One thing to know before reading on: I do not run a third-party wallbox myself. The caravan guide next to this one is a measured setup; this one is a template. The thresholds below are starting points with the reasoning behind each, not values I have watched a car charge on. Where a number depends on your hardware, the text says so.

## What this solves

A PowerOcean does surplus management on its own, for its own battery. Every watt the roof produces beyond the house load goes into the battery first, and only what the battery cannot take, because it is full or because its charge power is capped, reaches the grid as export. The EcoFlow app shows this as the familiar picture: solar up, battery charging, export near zero until the afternoon.

A car on a wallbox does not appear in that picture. Left alone, a wallbox charges at whatever it is set to, from whatever source is there, and on a sunny noon that is mostly the grid, because the battery has already claimed the surplus. The automation in this guide makes the car take the surplus instead, and it decides between the car and the home battery with one number: the battery state of charge at which the car is allowed to take the battery's share.

## The PowerPulse case first

If your wallbox is an EcoFlow PowerPulse or PowerPulse 2 on the same account, stop here and use the app. The PowerOcean already drives that wallbox from surplus: in the app it is the setting called "Prioritize controllable devices", and this integration exposes the same slider as the number **Solar Surplus Threshold** on the PowerOcean device. Above that battery SoC, surplus goes to the wallbox before it goes into the battery. Set it, and the PowerOcean does everything this guide builds by hand, with the one advantage no automation has: it decides on the device, on the same readings, without a round trip through the cloud.

What this integration adds for a PowerPulse 2 is two buttons, **Wallbox Start Charging** and **Wallbox Stop Charging**, confirmed from the wallbox's own report. They are for a manual start and stop, or for an automation that has a reason the surplus logic does not know, such as a departure time. Do not build the surplus loop below on top of them: two controllers on one wallbox fight, and the one on the device wins.

## What you need

| Item | Why |
|---|---|
| A PowerOcean with account sign-in set up in this integration | The four readings below update every 3 seconds on that path. On developer keys they update every 30 seconds, which still works for the start and stop decision, but the current regulation in the package will lag behind clouds |
| A wallbox with a Home Assistant integration that offers a charging switch, or a charging current number, or both | The package writes to a switch and a number. Which of the two your wallbox exposes decides which half of the package applies, see "Adapting to your wallbox" |
...
The readings the package uses, all on the PowerOcean device:
...
The PowerOcean charges its battery before it exports, so on most days the export reading stays near zero until the battery is nearly full, and a loop that only watches export would not start the car before mid-afternoon. That is the right behaviour for some households and the wrong one for others, and the difference is a single threshold:

```
budget = grid_export_power + (battery_charge_power if battery_soc >= handover_soc else 0)
```

Below `handover_soc` the car gets only what would otherwise be exported. At or above it, the car also gets what the battery is currently taking, which the battery will stop taking as soon as the car draws it, because the PowerOcean sees less surplus. Nothing is written to the PowerOcean for this; the PowerOcean simply responds to the house load rising.

A starting point is 80 %. Set it higher if you want the evening covered from the battery before the car sees any surplus, lower if the car is the thing you want charged. The package keeps this number in an input helper so it can be changed from a dashboard without editing YAML.

## Why the thresholds are not small

A wallbox cannot charge slowly below a floor. The charging standard sets the minimum at 6 A, and at 230 V that is about 1.4 kW on one phase and about 4.1 kW on three. A budget below that floor cannot be used; the car either charges at the minimum, partly from the grid, or not at all. So the start threshold in the package is the floor plus a margin, and the numbers per phase count are:
...
The stop threshold is lower than the start threshold, and both are held for minutes rather than acted on at once. A passing cloud takes the export to zero for a minute; that is not a reason to open the relay, and a contactor that opens and closes every few minutes wears out and annoys the car. The package waits 3 minutes above the start threshold before starting and 5 minutes below the stop threshold before stopping. Adjust to taste, not below one minute.

## The package

Save this as a package, for example `packages/ev_surplus.yaml`, with `homeassistant: packages: !include_dir_named packages` in `configuration.yaml`. Then replace every entity id marked `# CHANGE` with your own. Developer Tools > States lists them.
...
The package assumes a switch for charging on and off and a number for the current in amperes. Wallbox integrations differ in three ways, and each one is a local edit:

- **Current is set through a service, not a number.** Replace the two `number.set_value` calls with the service the integration documents, and drop the change-only condition in the regulate automation or rebuild it on the integration's current sensor.
- **No current control at all, only on and off.** Delete the regulate automation and the `min_a`, `max_a` and `watts_per_amp` helpers. Set the start threshold to what the wallbox draws at its fixed current plus a margin. This is the least efficient shape, because every cloud either stops the car or pulls the difference from the grid, but it works.
- **Phase switching.** If the wallbox can switch between one and three phases, switch to one phase in the start action and back to three in the regulate automation once `target_a` sits at `max_a` for a while. The details depend on how the integration exposes it, and a wallbox that switches phases under load needs the pause the manufacturer specifies.
...
- **The PowerOcean's own Solar Surplus Threshold is still in play.** It only concerns EcoFlow devices, so a third-party wallbox is not affected by it. But the app enforces `Backup Reserve <= Solar Surplus Threshold`, and this integration enforces the same, so a backup reserve set high pushes the surplus threshold with it. Neither touches the loop above; they are mentioned so nobody looks for the car's behaviour in the wrong slider.
- **The battery takes the surplus back.** Below the hand-over SoC the loop does not touch the battery's share, so on a day when the battery is empty in the morning the car starts late. That is the hand-over number doing its job, not a bug. If you want the car first, lower it.
- **The delays are the protection.** Shortening `delay_on` below a minute turns a passing cloud into a relay cycle. Lengthening the stop delay is the cheaper way to ride out clouds than lowering the stop threshold, because a low stop threshold lets the car run on grid power for as long as the delay.
...
- Vehicle state of charge and a charge limit. Most car integrations expose both, and a condition `car_soc < car_limit` on the start automation is the whole addition.
- Charging by tariff, or a departure-time top-up from the grid. Both are conditions on the same switch and coexist with the surplus loop as long as only one of them may turn the wallbox on at a time.
