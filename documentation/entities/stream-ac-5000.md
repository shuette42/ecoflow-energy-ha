...
With units linked, the strings are the unit's own: the device reports one set of string readings per linked unit, each stamped with that unit's serial, and a unit publishes only the set that carries its own. On a linked pair the reporting travels on one unit's connection, and which one is not fixed, so a set stamped with the other unit's serial is handed to that unit and published there. When both connections carry a unit's set, the unit's own connection is preferred for 60 seconds after it last delivered one, because the copy that travels on the neighbour's connection is rounded to whole watts. A string absent from a set is read as 0 W only while the strings present account for the set's total, which is how the device reports an idle string; a relayed set whose total says otherwise leaves that sensor at its last reading. The `linked_units` section of a diagnostics download counts the sets each unit saw on its own connection, handed over, received and held, and shows the state of charge the unit's own set carries next to the unit's precise state of charge from the same message, which is how a masked download can still tell whether the set stamped with the unit's serial is the unit's own.
