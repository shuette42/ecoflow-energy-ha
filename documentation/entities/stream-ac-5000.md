...
| Max Grid Input Power | W | diagnostic | enabled | The app's "Max grid input power", and the setting that actually governs grid charging. Read-only here, set in the app under "netzgekoppelte Eingangsleistung" |
...
- **A setpoint above one of the app's power limits is clamped, not refused.** Max Grid-tied Output Power and Max Grid Input Power report them. The output one is also a control here; the input one is set in the app only, and it is what a grid charge is actually held to. Above both sits an account ceiling that only EcoFlow can raise, and the output control offers that ceiling as its upper end rather than the model's rated 2500 W, because a range above it would do nothing.
