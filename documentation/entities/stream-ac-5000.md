...
With units linked, the strings are the unit's own: the device reports one set of string readings per linked unit, each stamped with that unit's serial, and a unit publishes only the set that carries its own. On a linked pair the reporting travels on one unit's connection, so the other unit's PV entities can stay absent even while its strings produce; that is the case the `linked_units` section of a diagnostics download describes, and handing those readings to the second unit is a separate step still open on #401.
