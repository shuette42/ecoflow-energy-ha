...
**Power Factor reads unknown while power flows.** The field is sent in every full frame and was zero throughout both recordings, on two separate installations, with power and current non-zero at the time. A power factor of zero while several hundred watts are flowing is not a value the meter can have measured, so the field is empty rather than measured, and an impossible number is worse than none. The entity is kept and reports nothing in that case. A zero while no power flows is left alone, because then there is nothing it contradicts, and the entity stays the cheapest way for an owner to see whether it ever moves.
