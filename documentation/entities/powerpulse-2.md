...
Created under the same condition as the number above: exactly one PowerOcean in the account, availability following its connection. The integration sends the mode alone, without the current or solar settings the app repeats beside it; the wallbox keeps those as they are. The confirmation comes from the heartbeat because that is the only place the wallbox reports the mode, and the heartbeat comes about once a minute, so a change can take up to a minute to return even when the wallbox took it at once.
