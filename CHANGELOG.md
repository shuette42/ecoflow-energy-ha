...
### Added
- The STREAM AC 5000 and STREAM 5000 (`ES22`/`ES21`) gain five PV readings: PV Total Power and PV 1-4 Power, named as the Stream Ultra and Ultra X name theirs. They come off the MPPT and sit alongside the existing Solar Power, which is unchanged: that one is the figure the device works out from the house flows and reports even with no PV wired to it, so the two are different readings rather than one replacing the other. Diagnostics captures from an ES21 with panels wired directly to it confirm the mapping - the total equals the sum of the four strings to the last decimal on every settled frame, and all four match what the EcoFlow app showed at the same moment (55, 79, 365 and 365 W against 54.62, 78.92, 364.24 and 369.71). A string that is producing nothing is left out of the message entirely rather than sent as a zero, which is why it is reported as 0 W here: on the earlier captures string 1 was idle and absent, and it appears with the sum still exact once it wakes up. All five appear only once a unit actually reports them, so a device with no PV is unaffected, and a string reading 0 W is shown rather than hidden, because that is the device stating the string is unwired or idle. (Ref #231)
