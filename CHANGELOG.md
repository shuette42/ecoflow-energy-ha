...
- Stream devices now report their telemetry in Standard mode as well, including per-string PV power for up to four strings, plus an optional kWh counter per string for the Energy Dashboard. Standard mode previously delivered the data but never mapped it, so no entities were created. PV 3 and PV 4 power are disabled by default because only the larger units use them, and the per-string energy counters are disabled by default because the existing solar energy sensor already covers the total. Enhanced mode continues to report the PV total only, since the protobuf stream does not break it down per string. (beta.19)
- Diagnostics now include the raw quota of Stream devices, alongside the existing Delta 3 raw quota, so field mappings can be confirmed and extended from a diagnostics download. (beta.19)
- Devices that are discovered but have no parser yet are now captured for diagnostics when the integration runs with EcoFlow account login. Such a device gets no entities, which also meant the data needed to add support for it could never be collected. A listen-only connection now records what the device sends and puts it into the diagnostics download, with the serial number masked and each message truncated. Nothing is ever sent to the device. This is the only route for models the EcoFlow Developer API refuses to expose. (beta.20)
...
### Changed
- The raw data capture for unsupported devices is now switched off by default and has to be enabled deliberately in the integration options. It only helps in one situation, when someone owns a device this integration does not support yet and wants to help add it, and it costs an extra connection per unsupported device in every installation that has one. Once enabled it runs for 24 hours and then switches itself off, so it cannot be left on and forgotten. The deadline is stored as a date, which means restarting Home Assistant does not hand out another 24 hours, and saving the options for an unrelated reason does not extend a running window either. Turning it off and on again does start a fresh one. (beta.21)
...
### Fixed
- An energy counter can no longer be frozen by a single impossible reading. Energy totals only ever count up, which is correct for a lifetime counter but meant that one wrong value took the counter with it: a PowerOcean Plus that briefly reported 54,501,280 kWh instead of 2,639 kWh kept showing that number long after the underlying decoding fault was fixed, because every correct reading afterwards was lower and therefore ignored. Readings beyond what any device can physically deliver are now rejected at every point they can enter, and an installation already carrying such a value drops it on the next restart and counts on from zero. Genuine lifetime counters, including decades of production, are unaffected. Anyone whose solar energy sensor showed an absurd figure should also delete the affected long-term statistics under Developer Tools, since a stored statistic cannot be corrected from the integration. (beta.24)
- PowerOcean Plus solar strings 3 and 4 now have entities. Since beta.22 the third string is decoded and shows up in diagnostics, but no sensor was ever created for it, so the value had nowhere to go and the entities stayed unavailable after a restart. Power, voltage and current are now available for both strings. They are disabled by default because ordinary PowerOcean units have two strings and would otherwise gain three permanently empty sensors: on a Plus unit, enable them under Settings, Devices, PowerOcean, Entities. Confirmed against live values from a PowerOcean Plus R374. (beta.24)
- PowerOcean grid phase current and active power are reported again. In beta.22 both were forced to zero on every PowerOcean, so a unit exporting more than 200 W on a phase showed 0 W and 0 A while voltage, reactive and apparent power stayed correct. The device describes each phase through two containers, one of which carries only voltage and frequency, and the missing entries in that one were filled with zeros that overwrote the real readings. A value the device actually sent is no longer replaced by a zero. Verified against a captured heartbeat from a PowerOcean HJ31. (beta.23)
- PowerOcean Plus (R371, R374, HJ3C) now reports all of its telemetry. These units answer a state request with a single reply that bundles up to 19 separate messages, but only the first one was ever read and everything after it was discarded without a trace. Most sensors therefore stayed empty, and the values that did arrive could be assembled from mismatched parts of the reply. Each message in a reply is now decoded on its own, which raises a tested R374 from 19 to 47 reported values. Units with a third solar string now report it as well, and reactive and apparent power are available per phase. Verified on a PowerOcean Plus against a second reference installation. (beta.22)
- A failed reconnect attempt no longer appears as an error in the log. A temporary name resolution failure produced a red error line for something the automatic retry fixed a moment later. Repeated failures are still reported, now as a warning, once attempts start piling up. (beta.21)
- The message about an unsupported device now explains how to help: switch on the raw data capture in the integration options and attach a diagnostics download to the issue. Previously it asked for an issue without saying what would actually make the device supportable. (beta.21)
- The raw data capture for unsupported devices no longer writes warnings or errors into the Home Assistant log. It reuses the regular MQTT client, which reports connection trouble at warning level, so a device that has no entities at all could fill the log with messages about a connection nobody asked for and nothing could be done about. Those messages are now debug-level for the capture connection only; regular devices report problems exactly as before. (beta.21)
- The raw data capture for unsupported devices is now genuinely listen-only, and its diagnostics output no longer contains account identifiers. The shared MQTT client requests a full state update from the device whenever it connects, which also applied to the capture connection, so a device that the integration deliberately does not support could still be addressed. The connection is now marked listen-only and blocks every outgoing message. Separately, the recorded topic names contained the EcoFlow account id, which was masked inside the captured messages but not in the topic strings; account id and certificate account are now masked there too. Neither problem could take effect in a released version, because the capture never got as far as connecting before this release. The diagnostics also state explicitly when no capture is running for a device instead of leaving the section out, which previously looked the same as an older version without the feature. (beta.21)
- The raw data capture for unsupported devices now actually records something. The listen-only connection was opened but its network loop was never started, so the connection never completed and no message ever arrived. Diagnostics showed an empty capture for every such device, which looks exactly like a device that sends nothing at all. Anyone who downloaded diagnostics for an unsupported model since the feature appeared should take a fresh one after updating. (beta.21)
...
- Controls no longer show a state the device rejected. The Smart Plug switch and the Delta switch and slider paths applied the new state before checking whether the command was sent, so a failed send displayed a wrong state for several seconds. (beta.7)
- Smaller fixes: diagnostics downloads now shorten device serial numbers to a 4-character prefix, enum sensors ignore unknown values from the device instead of breaking their state updates, only the main battery SoC drives the device-card header on the Delta 2 Max, and every lifetime counter is covered by the never-decrease guard. (beta.7)
- Negative readings from Smart Plug and Stream AC Pro no longer appear as astronomically large numbers. The binary telemetry encodes values below zero in a form the decoder read as unsigned, so a temperature of -5 C surfaced as roughly 18 quintillion. Signed values now decode correctly. (beta.7)
- Delta 2 Max no longer breaks its state sensors when the firmware reports a charging or charger state the integration does not know yet. Unknown values previously passed through as raw numbers and made Home Assistant reject every state update for that sensor; they are now skipped, keeping the sensor on its last valid state. (beta.7)
- The Delta 3 Max Plus remaining charge and discharge time now clear to unknown when their direction becomes inactive. The clear was sent but the sensor fell back to the last stored value, so an idle unit could keep showing a stale runtime. (beta.7)
- PowerOcean grid detection now considers all three phase voltages instead of only phase A, so a setup fed on phase B or C, or a single-phase outage on A, reports the grid state correctly. (beta.7)
- The Delta 2 Max configured AC output voltage is now also available when polling over HTTP, matching what the push channel already reported. (beta.7)
- Internal parser hardening: malformed or truncated binary frames and unexpected message envelopes are now discarded cleanly instead of raising, work-mode values delivered as text map correctly again, and PowerOcean battery pack numbering no longer depends on the order the cloud returns them. (beta.7)
- Expired live-connection credentials now recover automatically. The MQTT library reports connection results in a format the integration could not process, which silently stopped the connection thread and broke the automatic credential refresh, so an expired login stayed broken until a restart. Authorization failures are now recognized correctly and trigger the refresh as designed. (beta.7)
- A forced credential refresh after an authorization failure no longer runs into the internal rate limit. The refresh was answered from the just-cleared cache, so it never fetched new credentials and discarded the working ones, which could lead to an unnecessary re-authentication prompt. The refresh now always fetches, and the previous credentials are kept until new ones arrive. (beta.7)
- Accounts registered on the global EcoFlow endpoint can now complete setup with EcoFlow account sign-in. Login tried both the EU and global hosts, but all follow-up requests went to the EU host only, so a global-account login succeeded and everything after it failed. All requests now stay on the host that accepted the login. (beta.7)
- Reconnect attempts after repeated connection failures now respect the documented 60 second maximum. The backoff multiplier was applied after the cap, stretching the effective wait to up to 120 seconds. (beta.7)
- Internal connection hardening: simultaneous reconnect triggers no longer race each other, refreshed credentials are applied to the live connection immediately, and the connection keepalive no longer feeds its own echo back into the data pipeline. (beta.7)
...
- Stream AC Pro Backup Reserve control now applies to the device. Changing the slider in Home Assistant had no effect because the write was sent on the device's acknowledgement channel instead of its configuration channel. It now uses the correct configuration path (field 102) and the value reaches the device. (beta.14, Ref #98)
...
- Delta 3 Max Plus now reports data in Enhanced mode. The device sends its status in a compact binary format that the integration recognized but discarded, so every entity stayed empty. Battery state of charge, input and output power, AC, solar, 12 V and USB port power, port flow states, charge and discharge state with remaining times, the SoC limits, the backup reserve and the switch states now update on the live connection, and they use the same entity IDs as in Standard mode, so switching between modes keeps your history and dashboards intact. (beta.6, Ref #100, #111)
- Delta 3 Max Plus switches and sliders now work in Enhanced mode too. Until now a control flipped in Enhanced mode snapped back, because commands were only sent on the Standard mode path. They are now sent on the live device connection instead, and the device confirms each one, so both modes offer the full set of controls. (beta.6, Ref #100, #111)
- Devices added to your EcoFlow account after the initial setup can now be added through the integration options when you signed in with your EcoFlow account. Previously the options screen only listed the devices selected during setup, so a newly bought device could only be added by deleting and re-creating the integration. If the account cannot be reached, the options screen keeps working and offers the devices you already had. (beta.6, Ref #100, #111)
