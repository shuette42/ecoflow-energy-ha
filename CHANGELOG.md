...
### Added
- Delta 3: the AC charge power is now adjustable from Home Assistant, together with the charge mode it depends on. The charge power slider covers 200 W to 2400 W in 100 W steps and does the same thing as the charge speed slider in the EcoFlow app, which means grid charging can be driven by a dynamic tariff or by solar surplus instead of by a fixed schedule. The two ship together because the app presents the charge speed slider under its custom power mode, so a power control without the mode next to it would leave anyone whose unit sits in Battery optimised or Silent unable to reach the setting they were actually after. What the device does with a wattage in those two modes was not measured, only that it accepts and reports one. Both settings are read back from the device's own report, so what Home Assistant shows is what the device is actually doing, and a value the device refuses is reported as a failure rather than left standing in the interface. They need the EcoFlow account sign-in, because the device reports both only on the live connection and never in the polled data, which leaves nothing to confirm a write against with developer keys. (Ref #111, #100, #181)
